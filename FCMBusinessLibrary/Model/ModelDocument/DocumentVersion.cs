...
namespace FCMMySQLBusinessLibrary.Model.ModelDocument
{
    public class DocumentVersion
    {
        ...
        private void AddSQLParameters(MySqlCommand command, string action)
        {
            ...
            command.Parameters.Add("@FKDocumentCUID", MySqlDbType.VarChar).Value = FKDocumentCUID;
            command.Parameters.Add("@IssueNumber", MySqlDbType.Decimal).Value = IssueNumber;
            ...
            command.Parameters.Add("@UpdateDateTime", MySqlDbType.DateTime, 8).Value = DateTime.Now;

            command.Parameters.Add("@UserIdUpdatedBy", MySqlDbType.VarChar).Value = Utils.UserID;

            if (action == MakConstant.SQLAction.CREATE)
            {
                command.Parameters.Add("@CreationDateTime", MySqlDbType.DateTime, 8).Value = DateTime.Now;
                command.Parameters.Add("@UserIdCreatedBy", MySqlDbType.VarChar).Value = Utils.UserID;
            }
        }
